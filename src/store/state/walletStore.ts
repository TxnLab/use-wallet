    ...
    persist(
      devtools((set, _get) => ({
        ...
        clearActiveAccount: (id: PROVIDER_ID) => {
          set((state) => {
            if (id === state.activeAccount?.providerId) state.activeAccount = null
          })
        },
        addAccounts: (accounts: Account[]) => {
          set((state) => {
            const uniqueAccounts = [...state.accounts, ...accounts].reduce<Account[]>(
              (uniqueAccounts, o) => {
                if (
                  !uniqueAccounts.some(
                    (obj) => obj.providerId === o.providerId && obj.address === o.address
                  )
                ) {
